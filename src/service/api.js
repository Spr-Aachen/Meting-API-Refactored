...
export default async (ctx) => {
    ...
    // determine cookie to use (from env or cookie file) if referrer allowed
    const referrer = ctx.req.header('referer') || ctx.req.header('referrer') || ''
    const allowed = isAllowedHost(referrer)
    const cookie = allowed ? await readCookieFile(server) : ''
