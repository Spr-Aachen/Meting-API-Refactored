...
export const request = async (method, url, data = {}, options) => {
    ...
    // if cookie provided as string like "k=v; k2=v2", parse into object
    if (typeof options.cookie === 'string') {
        const obj = {}
        options.cookie.split(';').forEach(s => {
            const kv = s.split('=')
            if (kv.length >= 2) {
...
export const map_song_list = (song_list) => {
    return song_list.songs.map(song => {
        ...
        const picUrl = song?.al?.picUrl || song.id
        return {
            title: song.name,
            author: artists.reduce((i, v) => ((i ? i + " / " : i) + v.name), ''),
            pic: picUrl.toString().replace(/^http:\/\//, 'https://'),
            url: song.id,
            lrc: song.id
        }
    })
